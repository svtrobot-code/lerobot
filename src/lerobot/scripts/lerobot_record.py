...
from lerobot.teleoperators import (  # noqa: F401
    ...
    openarm_leader,
    openarm_mini,
    openarm_rosbridge_bimanual as openarm_rosbridge_bimanual_teleop,
...
class RecordConfig:
    ...
    resume: bool = False
    # Show a live, unrecorded preview and require Enter before every episode.
...
def record(
    ...
    try:
        ...
        if cfg.wait_for_enter and listener is None:
            raise RuntimeError(
                "wait_for_enter=true requires an interactive keyboard input backend. "
                "Run lerobot-record from an interactive terminal."
            )
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                if cfg.wait_for_enter:
                    events["waiting_for_start"] = True
                    events["start_recording"] = False
                    events["exit_early"] = False
                    log_say(
                        f"Ready for episode {episode_index}. Focus the recording terminal and press ENTER to start",
                        cfg.play_sounds,
                    )
                    # Keep observations, teleoperation and visualization live while waiting,
                    # but do not attach a dataset so none of these preview frames are saved.
                    record_loop(
                        robot=robot,
                        events=events,
                        fps=cfg.dataset.fps,
                    ...
                    events["waiting_for_start"] = False
                    events["exit_early"] = False
