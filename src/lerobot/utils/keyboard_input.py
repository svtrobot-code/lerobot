...
def init_keyboard_listener():
    ...
    * otherwise no listener (non-interactive / piped runs) — recording relies on
    ...
    Both backends accept the same controls: Enter starts an episode while a caller is
    waiting, Right/Left/Esc control an active recording, and the single-byte letter
    equivalents are ``n`` (next), ``r`` (re-record) and ``q`` (quit). The letters are the
    most reliable choice over high-latency SSH/VNC links, where arrow-key escape sequences
    can be split, delayed, or intercepted by the terminal.
    ...
    def on_key(name: str) -> None:
        key = name.lower()
        if events["waiting_for_start"]:
            if key == "enter":
                print("Enter key pressed. Starting episode...")
                events["start_recording"] = True
                events["exit_early"] = True
            elif key in ("esc", "q"):
                apply_recording_control("esc", events)
            # Episode navigation is deliberately ignored until recording starts.
            return
        elif key in ("right", "n"):
    ...
    listener = create_key_listener(
        on_key,
        controls_help="Enter=start, Right/Left/Esc, or n=next, r=re-record, q=quit",
    )
